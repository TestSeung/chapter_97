...
@Repository
public class AirlineTicketJdbcTempleDao implements AirlineTicketRepository {
    private JdbcTemplate jdbcTemplate;

    public AirlineTicketJdbcTempleDao(@Qualifier("jdbcTemplate2") JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }
    static RowMapper<AirlineTicket> userEntityRowMapper=((rs, rowNums)->
            new AirlineTicket(
                    rs.getInt("ticket_id"),
    ...
    static RowMapper<AirlineTicketAndFlightInfo> userEntityRowMapper2=((rs, rowNums)->
    ...
    public List<AirlineTicket> findAllAirlineTicketWithPlaceAndTicketType(String likePlace, String ticketType) {
        return jdbcTemplate.query("SELECT *" +
                                        "FROM airline_ticket " +
                                    "   WHERE arrival_loc = ? " +
                                        " AND ticket_type =?", userEntityRowMapper, likePlace, ticketType);
    }
    ...
    public List<AirlineTicketAndFlightInfo> findAllAirLineTicketAndFlightInfo(Integer airlineTicketId) {
        return jdbcTemplate.query("SELECT A.ticket_id,A.tax,A.total_price,F.flight_price, F.charge " +
                                        ...
                                        "WHERE A.ticket_id = ?", userEntityRowMapper2, airlineTicketId );
    }
}
